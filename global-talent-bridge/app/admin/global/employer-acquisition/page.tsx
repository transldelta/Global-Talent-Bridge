...
import { RunGEAAgentButton } from './_components/RunGEAAgentButton'
...
const TYPE_STYLES: Record<string, string> = {
  hospital_group:       'bg-red-900/20 text-red-300 border-red-800/30',
  care_home_group:      'bg-pink-900/20 text-pink-300 border-pink-800/30',
  it_company:           'bg-blue-900/20 text-blue-300 border-blue-800/30',
  logistics_company:    'bg-yellow-900/20 text-yellow-300 border-yellow-800/30',
  industrial_company:   'bg-orange-900/20 text-orange-300 border-orange-800/30',
  recruiter:            'bg-purple-900/20 text-purple-300 border-purple-800/30',
  staffing_agency:      'bg-teal-900/20 text-teal-300 border-teal-800/30',
  employer_association: 'bg-green-900/20 text-green-300 border-green-800/30',
}

const TYPE_LABELS: Record<string, string> = {
  hospital_group:       '🏥 Krankenhaus-Gruppe',
  care_home_group:      '🏠 Pflegeheim-Gruppe',
  it_company:           '💻 IT-Unternehmen',
  logistics_company:    '🚛 Logistik',
  industrial_company:   '🏭 Industrie',
  recruiter:            '🎯 Recruiter',
  staffing_agency:      '🤝 Personalvermittlung',
  employer_association: '🏛️ Arbeitgeberverband',
}
...
function formatEmployers(n: number): string {
...
export default async function EmployerAcquisitionPage() {
    ...
    <>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-1 text-sm text-gray-500">
              ...
              <span>·</span>
              <Link href="/admin/global/employer-outreach" className="hover:text-gray-400">Outreach-Empfehlungen</Link>
            </div>
            <h1 className="text-2xl font-bold text-white">🏢 Employer Acquisition</h1>
            <p className="text-gray-400 text-sm mt-1">
              Globale Arbeitgeberquellen — analysiert, priorisiert, nicht automatisch kontaktiert
            </p>
          </div>
          <RunGEAAgentButton />
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          ...
          <KpiCard label="Erreichbare Arbeitgeber" value={formatEmployers(totalEmployers)} color="green" />
        </div>
        ...
        {all.length === 0 ? (
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-12 text-center text-gray-500">
            ...
            <div>Noch keine Quellen. GEA-Agent starten um Analyse zu beginnen.</div>
          </div>
        ) : (
            ...
            {all.map((src) => (
              ...
              >
                ...
                <div className="flex justify-between text-xs mb-3">
                  <span className="text-gray-500">Erreichbare Arbeitgeber</span>
                  <span className="text-white font-semibold">{src.estimated_employers.toLocaleString('de-DE')}</span>
                </div>
        ...
        {/* Compliance Note */}
        <div className="mt-8 p-4 bg-emerald-900/10 border border-emerald-800/30 rounded-xl text-xs text-emerald-300/70">
          🔒 <strong className="text-emerald-300">Compliance-Hinweis:</strong> Arbeitgeberquellen werden nur für
          interne Strategieentwicklung genutzt. Kein automatisches Anschreiben. Kein automatisches Scraping.
          Alle Outreach-Aktionen müssen manuell durch den Admin initiiert und freigegeben werden.
        </div>
